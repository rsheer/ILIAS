...
abstract class ilObjContainerDAV extends ilObjectDAV implements Sabre\DAV\ICollection
{
    ...
    public function createFile($name, $data = null)
    {
        ...
        else 
        {
            ...
            if(/*$name == ilFileUtils::getValidFilename($name)*/true)
            {
    ...
    public function getChildren()
    {
        ...
        ilLoggerFactory::getLogger('WebDAV')->debug(get_class($this). ' ' . $this->obj->getTitle() ." -> return array with ". count($child_nodes) . "childs");
        return $child_nodes;
    }
        ...
        foreach($this->tree->getChildIds($this->obj->getRefId()) as $child_ref)
        {
            ...
            if(self::_isDAVableObject($child_ref))
            {
                ...
                if(ilObject::_lookupTitle($obj_id) == $name)
                {
                    ...
                    if($this->access->checkAccess("read", '', $child_ref))
                    {
                        ilLoggerFactory::getLogger('WebDAV')->debug(get_class($this). ' ' . $this->obj->getTitle() ." -> child '$name'  exists");
                        return true;
                    }
                }
            }
